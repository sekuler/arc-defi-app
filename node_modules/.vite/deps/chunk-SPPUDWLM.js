...
} from "./chunk-33LA42AK.js";
import {
  init_sha2,
  sha256,
  sha384,
  sha512
} from "./chunk-6WZFPAQN.js";
import {
  init_sha3,
  keccak_256
} from "./chunk-KKEVZN5F.js";
import {
  __export
...
function assertEip712Transaction(transaction) {
  const { chainId, to, from: from20, paymaster, paymasterInput } = transaction;
  ...
  if (from20 && !isAddress(from20))
    throw new InvalidAddressError({ address: from20 });
...
function serializeTransactionEIP712(transaction) {
  const { chainId, gas, nonce: nonce2, to, from: from20, value, maxFeePerGas, maxPriorityFeePerGas, customSignature, factoryDeps, paymaster, paymasterInput, gasPerPubdata, data } = transaction;
...
function transactionToMessage(transaction) {
  const { gas, nonce: nonce2, to, from: from20, value, maxFeePerGas, maxPriorityFeePerGas, factoryDeps, paymaster, paymasterInput, gasPerPubdata, data } = transaction;
  return {
    txType: 113n,
    from: BigInt(from20),
    to: to ? BigInt(to) : 0n,
...
function serializeTransactionDeposit(transaction) {
  ...
  const { sourceHash, data, from: from20, gas, isSystemTx, mint: mint2, to, value } = transaction;
  const serializedTransaction = [
    sourceHash,
    from20,
    to ?? "0x",
...
function assertTransactionDeposit(transaction) {
  const { from: from20, to } = transaction;
  if (from20 && !isAddress(from20))
    throw new InvalidAddressError({ address: from20 });
  if (to && !isAddress(to))
    throw new InvalidAddressError({ address: to });
...
function verify2(options) {
  ...
  return secp256r12.verify(signature, payload instanceof Uint8Array ? payload : fromHex(payload), toHex3(publicKey).substring(2), { lowS: true, ...hash5 ? { prehash: true } : {} });
}
...
(function(decodeCursor2) {
  ...
  function getFloat16(bits) {
    ...
    if (exponent === 0) {
      ...
      return sign6 ? -value2 : value2;
    }
    if (exponent === 31) {
      if (fraction === 0)
        return sign6 ? -Infinity : Infinity;
      return NaN;
    }
    const value = 2 ** (exponent - 15) * (1 + fraction / 1024);
    return sign6 ? -value : value;
  }
...
function assert4(config) {
  ...
  for (const owner of owners) {
    if (!validate3(owner.owner) || toBigInt(owner.owner) === 0n)
...
function getAddress(value) {
  ...
  return from2(slice(hash5, 12, 32));
}
function getSignPayload2(value) {
  ...
  return keccak2562(concat2(fromString2(signatureDomain), from(payload), account, genesisConfigId));
}
function toId(config) {
  assert4(config);
  const id = keccak2562(concat2(fromString2(configDomain), padLeft2(config.salt ?? zeroSalt, 32), fromNumber(config.threshold, { size: 4 }), fromNumber(config.owners.length, { size: 4 }), ...config.owners.flatMap((owner) => [
    owner.owner,
    fromNumber(owner.weight, { size: 4 })
  ])));
  if (toBigInt(id) === 0n)
...
function toTuple2(config) {
  ...
  return [salt, fromNumber(config.threshold), owners];
}
...
function assert5(envelope) {
  ...
  if (type2 === "p256") {
    ...
    const missing = [];
    if (typeof ((_a2 = p2563.signature) == null ? void 0 : _a2.r) !== "bigint")
...
function from8(value, options) {
  ...
  if (type2 === "multisig") {
    ...
    return {
      ...
      account,
      genesisConfigId,
      signatures: rest.signatures.map((signature) => from8(signature)),
      ...
      ...initSource ? { init: from7(initSource) } : {},
      type: type2
    };
  }
  return {
...
function fromRpc2(envelope) {
  ...
  if (envelope.type === "keychain" || "userAddress" in envelope && "signature" in envelope) {
    ...
    return {
      ...
      inner: fromRpc2(keychain.signature),
  ...
  if (envelope.type === "multisig" || "account" in envelope && "configId" in envelope && "signatures" in envelope) {
    ...
    return {
      ...
      ...multisig.init ? { init: from7(multisig.init) } : {}
    };
  }
...
function serialize(envelope, options = {}) {
  ...
  if (type2 === "multisig") {
    const multisig = envelope;
    return concat2(serializedMultisigType, fromHex2([
      multisig.account,
...
function sortMultisigApprovals(value) {
  ...
  const digest = getSignPayload2("genesisConfig" in value && value.genesisConfig ? { payload, genesisConfig: value.genesisConfig } : {
    payload,
    account: value.account,
    genesisConfigId: value.genesisConfigId
  });
  return signatures.map((signature) => ({
    key: toBigInt(extractAddress({ payload: digest, signature })),
    signature
...
function verify5(signature, parameters2) {
  ...
  if (!address2)
    return false;
  const envelope = from8(signature);
  if (envelope.type === "secp256k1") {
    if (!address2)
      return false;
    return verify({
  ...
  if (envelope.type === "p256") {
    ...
    if (!isEqual2(envelopeAddress, address2))
      return false;
    return verify2({
...
function hash(authorization, options = {}) {
  ...
  return keccak2562(concat2("0x05", fromHex2(toTuple3(presign ? {
...
function compute(value) {
  const hash5 = keccak2562(encode(from4("address, bytes32"), [
...
// node_modules/ox/_esm/tempo/Channel.js
var channelIdParameters = from4("address, address, address, address, bytes32, address, bytes32, address, uint256");
var domainSeparatorParameters = from4("bytes32, bytes32, bytes32, uint256, address");
var voucherHashParameters = from4("bytes32, bytes32, uint96");
...
function from12(authorization, options = {}) {
  ...
  if (options.signature)
    return {
      ...resolved,
      signature: from8(options.signature)
    };
  return resolved;
}
...
var transferBlocked = from14("event TransferBlocked(address indexed token, address indexed receiver, uint64 indexed blockedNonce, uint256 amount, uint8 receiptVersion, bytes receipt)");
function decode4(receipt) {
...
function fromLog(log) {
  const { receipt } = decode3(transferBlocked, log);
  return receipt;
}
...
function toRpc5(transaction, _options) {
  ...
  if (typeof transaction.blockTimestamp === "bigint")
    rpc.blockTimestamp = fromNumber(transaction.blockTimestamp);
...
function fromRpc10(request) {
  ...
  if (typeof request.type !== "undefined")
    request_.type = fromRpcType[request.type] || request.type;
...
function fromRpc11(request) {
  ...
  const request_ = fromRpc10(rest);
  if (typeof request.type !== "undefined")
    request_.type = fromRpcType2[request.type] || request_.type;
  ...
  if (request.keyAuthorization)
    request_.keyAuthorization = fromRpc4(request.keyAuthorization);
  ...
  if (!transaction.from && signatureEnvelope) {
    try {
      transaction.from = extractAddress({
        payload: getSignPayload5(from16(transaction)),
        signature: signatureEnvelope,
        root: true
      });
    } catch {
...
function from16(envelope, options = {}) {
  ...
  return {
    ...envelope_,
    ...signature ? { signature: from8(signature) } : {},
...
function serialize4(envelope, options = {}) {
    ...
    if (options.format === "feePayer" && signature) {
      ...
      if (sig.type === "secp256k1")
        return recoverAddress({
          payload: getSignPayload5(from16(envelope)),
  ...
  const serialized = [
    ...
    ...signature ? [serialize(from8(signature))] : []
  ];
...
function from17(value) {
  return from2(concat2(toFixedHex(value.masterId, 4), magic, toFixedHex(value.userTag, 6)));
}
...
async function resolveNode() {
  ...
  const { wasmBase64 } = await import("./mine.wasm-NX3N2Z6O.js");
...
var fieldsSize = 29;
var version = 0;
function from18(authentication, options = {}) {
...
function getFields(authentication) {
  return concat2(fromNumber(version, { size: 1 }), fromNumber(authentication.zoneId, { size: 4 }), fromNumber(authentication.chainId, { size: 8 }), fromNumber(authentication.issuedAt, { size: 8 }), fromNumber(authentication.expiresAt, { size: 8 }));
}
function getSignPayload6(authentication, options = {}) {
...
function serialize5(authentication, options = {}) {
  ...
  return concat2(serialize(from8(signature)), getFields(authentication));
}
...
function resolveAccessKey(accessKey) {
  ...
  if ("publicKey" in accessKey && accessKey.publicKey)
    return {
      accessKeyAddress: fromPublicKey(fromHex3(accessKey.publicKey)),
...
@noble/curves/esm/p256.js:
  (*! noble-curves - MIT License (c) 2022 Paul Miller (paulmillr.com) *)
*/
//# sourceMappingURL=chunk-SPPUDWLM.js.map
