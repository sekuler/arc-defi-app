...
interface SwappingProvider {
     ...
     * ```
     */
    swap<TFromAdapterCapabilities extends AdapterCapabilities>(params: ServiceSwapParams<TFromAdapterCapabilities>): Promise<SwapResult>;
}
/**
 * Context passed between swap steps containing execution artifacts.
 *
 * @remarks
 * This context object accumulates execution data as the swap process
 * progresses, allowing later steps to access results from earlier steps.
 * Following the same pattern as CCTP v2's StepContext, this accumulates
 * step outputs (service responses, transaction hashes) needed by subsequent steps.
 *
 * The `estimateResult` includes estimated outputs, fees, and routing information.
 * While the full response is stored here, step executors should still receive
 * request parameters directly via function arguments rather than extracting them
 * from this context.
 *
 * Request parameters (e.g., ServiceSwapParams) should be passed directly
 * to step executors as function arguments, not extracted from this context.
 *
 * @example
 * ```typescript
 * import type { SwapContext } from '@circle-fin/provider-stablecoin-service-swap'
 *
 * // Context after fetching swap quote
 * const context: SwapContext = {
 *   estimateResult: {
 *     stopLimit: '99500000',
 *     estimatedOutput: '99.5',
 *     fees: [{ token: 'USDC', amount: '0.002' }]
 *   }
 * }
 *
 * // Context after swap execution
 * const updatedContext: SwapContext = {
 *   estimateResult: context.estimateResult,
 *   swapTxHash: '0x1234567890abcdef...'
 * }
 * ```
 */
interface SwapContext {
    /**
     * The estimate result from the Stablecoin Service swap API.
     *
     * @remarks
     * Populated after receiving a quote or swap response from the service.
     * Contains estimated outputs, fees, and routing information needed by
     * subsequent steps.
     */
    estimateResult?: EstimateResult;
    /**
     * The transaction hash from the swap execution step.
     *
     * @remarks
     * Populated after successfully submitting the swap transaction on-chain.
     * Required for confirmation, tracking, and potential retry scenarios.
     */
    swapTxHash?: string;
}

/**
 * Configuration options for the Stablecoin Service Swap provider.
 *
 * @remarks
 * This interface defines configurable aspects of the provider instance:
 * - customFee: Optional developer fee settings for collecting fees on swaps
 *
 * The custom fee is applied to all swap operations executed through the provider
 * instance unless overridden at the swap level via `ServiceSwapConfig.customFee`.
 *
 * Note: Kit key authentication must be provided inline per operation via `config.kitKey`.
 *
 * @example
 * ```typescript
 * import { StablecoinServiceSwapProviderConfig } from '@circle-fin/provider-stablecoin-service-swap'
 *
 * // Configure provider with 1 USDC fee
 * const config: StablecoinServiceSwapProviderConfig = {
 *   customFee: {
 ...
 * @example
 * ```typescript
 * // No custom fee (default behavior)
 * const configNoFee: StablecoinServiceSwapProviderConfig = {}
 * ```
 */
interface StablecoinServiceSwapProviderConfig {
    /**
     * Optional developer fee configuration applied to the swap.
     *
     * @remarks
     * Allows collecting a custom fee on each swap operation. The fee is taken
     * from the swap amount and sent to the specified recipient address.
     */
    readonly customFee?: {
        /**
         * Custom fee amount to collect in base units (stringified bigint).
         *
         * @example '1000000' for 1 USDC fee (6 decimals)
         */
        readonly amount: string;
        /**
         * Wallet address that receives the custom fee.
         *
...
export type { AllowanceStrategy, EstimateResult, ExecutedTransaction, ServiceSwapConfig, ServiceSwapFee, ServiceSwapParams, StablecoinServiceSwapProviderConfig, SwapContext, SwapResult, SwappingProvider, TokenAmount };
