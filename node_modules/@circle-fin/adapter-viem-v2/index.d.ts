...
/**
 * Result of a batched EIP-5792 `wallet_sendCalls` execution.
 *
 * Contains the wallet-assigned batch identifier and individual receipt
 * information for each call in the batch, obtained by polling
 * `wallet_getCallsStatus` until completion.
 *
 ...
 *   batchId: '0xabc123',
 *   receipts: [
 *     { txHash: '0x111...', status: 'success' },
 *     { txHash: '0x222...', status: 'success' },
 *   ],
 * }
 * ```
 */
interface BatchExecuteResult {
    /** The wallet-assigned identifier for the batched call bundle. */
    batchId: string;
    /** Per-call receipt information in the same order as the submitted calls. */
    receipts: {
        /** The on-chain transaction hash for this call. */
        txHash: string;
        /** Whether the call succeeded or failed on-chain. */
        status: 'success' | 'error';
    }[];
    /**
     * The error that occurred during polling, if any.
     *
     * Present when `receipts` is empty due to a polling timeout or failure
     * after the batch was already submitted. Callers can inspect this to
     * distinguish "wallet doesn't support status polling" (`error` is
     * `undefined`, `receipts` is `[]`) from "polling timed out" (`error`
     * is a {@link KitError}).
     */
    error?: unknown;
    /**
     * The raw EIP-5792 `statusCode` returned by `wallet_getCallsStatus`
     * when a terminal status was reached.
     *
     * Per the EIP-5792 specification, valid terminal codes are:
     * - `200` — batch confirmed onchain
     * - `400` — batch failed offchain (not included onchain)
     * - `500` — batch reverted completely onchain
     * - `600` — batch reverted partially onchain
     *
     * Absent when polling timed out before a terminal status was reached,
     * when the wallet does not support status polling, or when the batch
     * was confirmed via a non-numeric `status` string returned by viem.
     *
     * @since 2.0.0
     */
    statusCode?: number;
...
/**
 * Options for {@link ViemAdapter.batchExecute}.
 *
 ...
 * }
 *
 * const result = await adapter.batchExecute(calls, Base, options)
 * ```
 */
interface BatchExecuteOptions {
    /**
     * Maximum time in milliseconds to wait for all calls to be confirmed.
     * @defaultValue 120_000 (2 minutes)
     */
    timeout?: number | undefined;
    /**
     * Interval in milliseconds between `wallet_getCallsStatus` polls.
     * @defaultValue 2_000 (2 seconds)
     */
...
declare class ViemAdapter<TAdapterCapabilities extends AdapterCapabilities = AdapterCapabilities> extends EvmAdapter<TAdapterCapabilities> {
     ...
     */
    private readonly cachedCapabilities;
    /**
     ...
     */
    private transformOverridesForViem;
    /**
     ...
     * ```
     */
    supportsAtomicBatch(chain: EVMChainDefinition): Promise<boolean>;
    /**
     ...
     * ```
     */
    batchExecute(calls: EvmCallData[], chain: EVMChainDefinition, options?: BatchExecuteOptions): Promise<BatchExecuteResult>;
    /**
...
export type { BatchExecuteOptions, BatchExecuteResult, ChainIdentifier, CreateAdapterFromPrivateKeyParams, CreateAdapterFromProviderParams, CreateViemAdapterFromPrivateKeyParams, CreateViemAdapterFromProviderParams, EIP2612Message, EIP2612NonceFetcher, EIP2612Options, PermitStandardName, Signature, TypedData, TypedDataField, ViemAdapterOptions };
