 ...
 */
const bridgeDestinationWithAddressSchema = adapterContextSchema$3
    .extend({
 ...
 */
createSwapRequestSchema.extend({
    /**
     * Circle Stablecoin Service API Key.
     * Must be a valid API key format.
     */
 ...
 * @internal
 */
const ALLOWANCE_THRESHOLD = MAX_UINT256 / 2n;
...
async function approveMaxIfNeeded(params) {
    ...
    const approvalPrepared = await adapter.prepareAction('token.approve', { tokenAddress, delegate, amount: MAX_UINT256 }, { chain, address });
 ...
 * @internal
 */
const getDefaultProviders = () => [new EarnServiceProvider()];
/**
 ...
 * ```
 */
async function deposit$1(context, params) {
    assertEarnParams(params, depositParamsSchema);
    const chain = resolveChainIdentifier(params.from.chain);
    params.from.adapter.validateChainSupport(chain);
