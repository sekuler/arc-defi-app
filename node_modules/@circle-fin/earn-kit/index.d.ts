 ...
 * ```
 */
declare function isResumableError(error: unknown): error is KitError;
/**
...
/**
 * Phase name of a multi-phase earn operation.
 *
 * - `fetchParams` — call the earn service to obtain signed execution
 *   parameters (also performs all input validation server-side).
 * - `approve` — issue a max ERC-20 approval to the adapter contract when
 *   the current allowance is below the SDK threshold. Skipped for
 *   claim-rewards operations.
 * - `execute` — submit the on-chain transaction via the adapter.
 *
 * @example
 * ```typescript
 * const step: EarnStepName = 'execute'
 * ```
 */
type EarnStepName = 'fetchParams' | 'approve' | 'execute';
/**
...
/**
 * Name of a multi-phase earn operation that emits step events.
 *
 * @example
 * ```typescript
 * const operation: EarnOperationName = 'deposit'
 * ```
 */
type EarnOperationName = 'deposit' | 'withdraw' | 'claimRewards';
/**
...
/**
 * Fields shared by every {@link EarnErrorTrace} variant.
 *
 * @example
 * ```typescript
 * const base: EarnErrorTraceBase = { provider: 'EarnService', steps: [] }
 * ```
 */
interface EarnErrorTraceBase {
    /** Name of the provider that produced the error (matches {@link EarningProvider.name}). */
    readonly provider: string;
    /** Final state of each phase at the time of failure. */
    readonly steps: readonly EarnStep[];
}
/**
 * Diagnostic and resume context attached to `KitError.cause.trace` when a
 * multi-phase earn operation fails.
 *
 * Discriminated on `operation`, which pairs with the matching `params` type so
 * {@link EarningProvider.retry} can resume the operation without re-validating
 * or re-casting. `steps` records the final state of each phase so consumers can
 * see where the operation stopped; `params` holds the original service params
 * and may contain in-memory references (such as the adapter), so it is not
 * serializable.
 *
 * @remarks
 * The `params` field is stored **non-enumerably** on the runtime trace object
 * (see {@link augmentEarnError}). It is accessible via `trace.params` but is
 * intentionally omitted from `JSON.stringify`, `console.log`, and telemetry
 * serialisers to prevent `config.kitKey` and the in-memory adapter from
 * appearing in logged errors.
 *
 * @example
 * ```typescript
 * try {
 ...
 *       console.log('phases:', trace.steps.map((s) => `${s.name}:${s.state}`))
 *     }
 *   }
 * }
 * ```
 */
type EarnErrorTrace = (EarnErrorTraceBase & {
    readonly operation: 'deposit';
    readonly params: DepositServiceParams;
}) | (EarnErrorTraceBase & {
    readonly operation: 'withdraw';
    readonly params: WithdrawServiceParams;
}) | (EarnErrorTraceBase & {
    readonly operation: 'claimRewards';
    readonly params: ClaimRewardsServiceParams;
});
/**
 ...
 * ```
 */
declare function isEarnErrorTrace(value: unknown): value is EarnErrorTrace;
...
declare class EarnKit {
     ...
     * ```
     */
    deposit<TFromAdapterCapabilities extends AdapterCapabilities = AdapterCapabilities>(params: DepositParams<TFromAdapterCapabilities>): Promise<EarnDepositResult>;
    /**
 ...
 * ```
 */
declare function deposit<TFromAdapterCapabilities extends AdapterCapabilities = AdapterCapabilities>(context: EarnKitContext, params: DepositParams<TFromAdapterCapabilities>): Promise<EarnDepositResult>;
 ...
 * ```
 */
declare const vaultQuerySchema: z.ZodObject<{
 ...
 * ```
 */
declare function assertEarnParams<T>(params: unknown, schema: z.ZodSchema<T>): asserts params is T;

export { Amount, Blockchain, EarnChain, EarnError, EarnKit, KitError, assertEarnParams, claimRewards, claimRewardsParamsSchema, createEarnKitContext, deposit, depositParamsSchema, getChainByEnum, getClaimRewardsQuote, getClaimRewardsQuoteParamsSchema, getDepositQuote, getDepositQuoteParamsSchema, getErrorCode, getErrorMessage, getPosition, getPositionParamsSchema, getSupportedChains, getVaults, getVaultsParamsSchema, getWithdrawalQuote, getWithdrawalQuoteParamsSchema, isEarnErrorTrace, isFatalError, isInputError, isKitError, isResumableError, isRetryableError, setExternalPrefix, vaultQuerySchema, withdraw, withdrawParamsSchema };
export type { AccruedRewardInfo, AdapterContext, AssetAmount, ChainDefinition, ChainIdentifier, ClaimRewardsParams, ClaimRewardsQuoteInfo, ClaimRewardsResult, ClaimedAmount, ClaimedRewardsResult, CollateralInfo, DepositParams, DepositQuoteInfo, EarnAccruedRewardInfo, EarnActionBase, EarnActionName, EarnActions, EarnAdapterContext, EarnAssetAmount, EarnChainDefinition, EarnChainIdentifier, EarnClaimRewardsQuoteInfo, EarnClaimRewardsResult, EarnClaimedAmount, EarnClaimedRewardsResult, EarnConfig, EarnDepositQuoteInfo, EarnDepositResult, EarnErrorTrace, EarnErrorTraceBase, EarnGetVaultsResult, EarnKitConfig, EarnKitContext, EarnOperationName, EarnOperationParams, EarnPositionInfo, EarnPositionPnLInfo, EarnServiceConfig, EarnStep, EarnStepBase, EarnStepName, EarnVaultInfo, EarnWithdrawResult, EarnWithdrawalQuoteInfo, EarningProvider, ErrorDetails, GetClaimRewardsQuoteParams, GetDepositQuoteParams, GetPositionParams, GetVaultsParams, GetVaultsResult, GetWithdrawalQuoteParams, NoClaimableRewardsResult, PositionInfo, PositionPnLInfo, Recoverability, VaultError, VaultInfo, VaultQuery, VaultRewardInfo, VaultWarning, WithdrawParams, WithdrawalQuoteInfo };
