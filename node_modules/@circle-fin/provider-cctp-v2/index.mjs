...
async function fetchUsdcFastBurnFee(sourceDomain, destinationDomain, isTestnet) {
    ...
    if (response.length === 0) {
        throw new KitError({
            ...
            cause: { trace: { sourceDomain, destinationDomain } },
        });
    }
    ...
    if (!fastTier) {
        throw new KitError({
            ...
            cause: {
                trace: { availableTiers: feeTiers.map((t) => t.finalityThreshold) },
            },
        });
    }
    ...
    if (Number.isNaN(feeValue) || !Number.isFinite(feeValue)) {
        throw new KitError({
            ...
            cause: { trace: { value: fastTier.minimumFee } },
        });
    }
    ...
    if (minimumFee < 0n) {
        throw new KitError({
            ...
            cause: { trace: { value: minimumFee.toString() } },
        });
    }
    return minimumFee;
}
...
function validateAndConvertFee(value, fieldName) {
    ...
    if (fee < 0n) {
        throw new KitError({
            ...
            cause: { trace: { value: fee.toString(), fieldName } },
        });
    }
    return fee;
}
...
const isForwardFee = (item) => {
    return (typeof item === 'object' &&
        ...
        (typeof item.med === 'string' ||
            typeof item.med === 'number') &&
...
const isForwardingFeeTier = (item) => {
    return (typeof item === 'object' &&
        ...
        'forwardFee' in item &&
        typeof item.finalityThreshold === 'number' &&
        (typeof item.minimumFee === 'string' ||
            typeof item.minimumFee === 'number') &&
        isForwardFee(item.forwardFee));
};
/**
...
function assertCCTPv2BridgeParams(params) {
    ...
    const isForwarderOnly = bridgeParams.destination.useForwarder === true &&
        !('adapter' in bridgeParams.destination && bridgeParams.destination.adapter);
    // Forwarder-only destinations require recipientAddress
    if (isForwarderOnly) {
        if (!bridgeParams.destination.recipientAddress?.trim()) {
            throw createValidationFailedError('recipientAddress', bridgeParams.destination.recipientAddress, 'recipientAddress is required when using forwarder without a destination adapter.');
        }
    }
...
function assertForwarderRouteSupport(source, destination, useForwarder) {
    if (useForwarder === true &&
        !destination.cctp?.forwarderSupported.destination) {
        throw new KitError({
            ...InputError.UNSUPPORTED_ROUTE,
            recoverability: 'FATAL',
            ...
            cause: {
                trace: { source: source.name, destination: destination.name },
            },
        });
    }
}
...
const isRelayerMintConfirmed = (obj) => {
    ...
    if ((message.forwardState === 'CONFIRMED' ||
        message.forwardState === 'COMPLETE') &&
        typeof message.forwardTxHash === 'string' &&
        message.forwardTxHash.trim().length > 0) {
        return true;
    }
...
const fetchRelayerMint = async (sourceDomainId, transactionHash, isTestnet, config = {}) => {
    ...
    try {
        ...
        if (error instanceof KitError &&
            error.name === 'NETWORK_RELAYER_FORWARD_FAILED') {
            throw new KitError({
                ...NetworkError.RELAYER_FORWARD_FAILED,
...
async function bridgeRelayerMint(params, context) {
    const step = { name: 'mint', state: 'pending', forwarded: true };
...
function createStepExecutors(useForwarder = false) {
    return [
        ...
        },
        {
            ...
            executor: useForwarder
                ? async (params, _provider, context) => {
                    if (!context) {
                        throw new KitError({
                            ...InputError.VALIDATION_FAILED,
                            recoverability: 'FATAL',
...
async function executeSteps(params, provider, result, startIndex, executors, useForwarder, invocation) {
    ...
    for (const { name, executor, updateContext } of executors.slice(startIndex)) {
        try {
            ...
            const newContext = updateContext?.(step);
            if (newContext) {
                stepContext = { ...stepContext, ...newContext };
            }
            dispatchStepEvent(name, step, provider, invocation);
            result.steps.push(step);
        }
...
class CCTPV2BridgingProvider extends BridgingProvider {
    ...
    supportsRoute(source, destination, token, useForwarder) {
        ...
        if (token === 'USDC') {
            ...
            if (useForwarder) {
                return (source.cctp.forwarderSupported.source &&
                    destination.cctp.forwarderSupported.destination);
            }
            return true;
        }
    ...
    async getMaxFee(params) {
        ...
        if (config?.maxFee !== undefined) {
            return { providerFee: BigInt(config.maxFee), forwarderFee: 0n };
        }
        ...
        if (transferSpeed === TransferSpeed.SLOW && !useForwarder) {
            return { providerFee: 0n, forwarderFee: 0n };
        }
        ...
        if (transferSpeed === TransferSpeed.SLOW && useForwarder) {
            ...
            return { providerFee: 0n, forwarderFee };
        }
        ...
        const [scaledBps, forwarderFee] = await Promise.all([
            ...
            useForwarder
                ? fetchForwardingFee(source.chain.cctp.domain, destination.chain.cctp.domain, source.chain.isTestnet, minFinalityThreshold)
                : Promise.resolve(0n),
        ]);
        ...
        return { providerFee, forwarderFee };
    }
    /**
    ...
    async burn(params) {
        ...
        if (useCustomBurn) {
            ...
            if (hookData) {
                return await source.adapter.prepareAction('cctp.v2.customBurnWithHook', { ...customBurnParams, hookData }, resolvedContext);
            }
        ...
        if (hookData) {
            return await source.adapter.prepareAction('cctp.v2.depositForBurnWithHook', { ...actionParams, hookData }, resolvedContext);
        }
