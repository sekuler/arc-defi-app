 ...
 *
 */
const swapChainIdentifierSchema = z.union([
 ...
 * ```
 */
const supportedSwapTokenSchema = z
    .string()
 ...
 * ```
 */
const getQuoteResponseSchema = z.object({
    ...
    quote: z.object({
        ...
        route: z.object({
            ...
            steps: z.array(z.unknown()), // Currently not populated (empty array)
        }),
    }),
...
const createSwapDeveloperFeeItemSchema = z.object({
    ...
    amount: feeAmountSchema,
    basis: z.enum(['inputAmount', 'estimatedAmount']),
});
/**
 * Zod schema for validating CreateSwapFees.
 */
const createSwapFeesSchema = z
    .object({
...
const createSwapTransactionInstructionSchema = z.object({
    ...
    data: z.string().min(1),
    value: z.string().min(1), // Hex string (e.g., '0x' or '0x0')
...
adapterSchema.extend({
    chainType: z.union([z.literal('evm'), z.literal('hybrid')]),
 ...
 * ```
 */
const solanaChainIdentifierSchema = z.union([
    z.literal('Solana'),
 ...
 * ```
 */
const solanaWalletProviderSchema = z
    .object({
    connect: z.function().returns(z.promise(z.object({
        publicKey: z.object({
            toString: z.function().returns(z.string()),
        }),
    }))),
    disconnect: z.function().returns(z.promise(z.void())),
    ...
    signTransaction: z.function().returns(z.promise(z.unknown())),
    signAllTransactions: z
        .function()
...
function assertIsSwapSupportedChainDefinition(chain) {
    if (!(chain.chain in SwapChain)) {
        const supported = Object.values(SwapChain).join(', ');
...
export { Blockchain, KitError, SwapChain, SwapKit, assertCustomFeePolicy, assertSwapParams, createSwapKitContext, customFeePolicySchema, estimate, getChainByEnum, getErrorCode, getErrorMessage, getSupportedChains, isFatalError, isInputError, isKitError, isRetryableError$1 as isRetryableError, removeCustomFeePolicy, resolveSwapParams, setCustomFeePolicy, setExternalPrefix, swap, swapParamsSchema };
