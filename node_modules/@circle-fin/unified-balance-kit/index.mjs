...
const unifiedBalanceChainIdentifierSchema = z.union([
...
// Zod schemas
// ---------------------------------------------------------------------------
const gatewayDomainSchema = z.object({
    chain: z.string(),
    network: z.string(),
    domain: z.number(),
    processedHeight: z.string().regex(/^\d+$/, 'processedHeight must be numeric'),
    burnIntentExpirationHeight: z
        .string()
...
const gatewayInfoSchema = z.object({
    version: z.number(),
    domains: z.array(gatewayDomainSchema),
});
// ---------------------------------------------------------------------------
...
const burnIntentResponseSchema = z.object({
...
const burnIntentSummarySchema = z.object({
    transferSpecHash: z.string(),
    maxBlockHeight: z.string(),
    maxFee: z.string(),
});
/** Schema for GET /v1/transfer/{id} response (transfer details). */
const gatewayTransferDetailsSchema = z.object({
    destinationDomain: z.number().optional(),
    status: z.enum(['pending', 'confirmed', 'finalized', 'failed', 'expired']),
    burnIntents: z.array(burnIntentSummarySchema).optional(),
 ...
 * ```
 */
const gatewayBalancesResponseSchema = z.object({
    token: z.enum(SUPPORTED_TOKENS$1),
...
const gatewayDepositEntrySchema = z.object({
    depositor: z.string(),
    domain: z.number(),
    transactionHash: z.string(),
    amount: z.string().pipe(createDecimalStringValidator({
        allowZero: true,
 ...
 */
const gatewayDepositsResponseSchema = z.object({
    token: z.string(),
    deposits: z.array(gatewayDepositEntrySchema),
});
/**
 ...
 */
const gatewayBalancesResponseUnionSchema = z.union([
 ...
 * \{ success: false, message \} is validated as an API error.
 */
const gatewayDepositsResponseUnionSchema = z.union([
    gatewayErrorResponseSchema,
 ...
 * @internal
 */
const supportedTokenSchema = z
    .string()
    .transform((v) => v.trim().toUpperCase())
    .pipe(z.enum(SUPPORTED_TOKENS));
...
const allowanceStrategySchema = z.enum(['approve', 'permit', 'authorize']);
/**
...
const depositForParamsSchema = depositParamsSchema
    ...
    .extend({
    depositAccount: z.string().min(1, 'Deposit account address is required'),
})
    .superRefine((data, ctx) => {
...
const spendSourceSchema = z
    .object({
    adapter: adapterSchema,
    allocations: z
        .union([allocationSchema, z.array(allocationSchema).min(1)])
        .optional(),
    address: z.string().optional(),
    sourceAccount: z.string().optional(),
})
    .strict();
const spendDestinationSchema = z
    .object({
    adapter: adapterSchema,
    chain: unifiedBalanceChainIdentifierSchema,
    recipientAddress: z.string().optional(),
    ...
    .strict();
const forwarderSpendDestinationSchema = z
    .object({
    chain: unifiedBalanceChainIdentifierSchema,
    ...
    .strict();
const spendDestinationUnionSchema = z.union([
    forwarderSpendDestinationSchema,
    spendDestinationSchema,
]);
const customFeeConfigSchema = z
    .object({
    recipientAddress: z.string().min(1, 'Fee recipient address is required.'),
    value: z
        .string()
    ...
    .strict();
const retryMintConfigSchema = z
    .object({
    attestation: z.string().min(1, 'Attestation is required for retry.'),
    signature: z.string().min(1, 'Signature is required for retry.'),
})
    .strict();
const spendConfigSchema = z
    .object({
...
const fromSchema = z.union([
    spendSourceSchema,
    z.array(spendSourceSchema).min(1),
]);
...
const sourcesSchema = z.union([
    balanceSourceSchema,
    z.array(balanceSourceSchema).min(1),
]);
/**
...
const getBalancesParamsSchema = z
    ...
    networkType: z.enum(['mainnet', 'testnet']).optional(),
})
    .strict();
 ...
 * - `token` — optional, must be a supported token if provided
 */
const delegateParamsBaseSchema = z
    .object({
...
const removeFundParamsSchema = z
    .object({
 ...
 */
const initiateRemoveFundParamsSchema = z
    .object({
    from: adapterContextSchema.strict(),
    amount: z.string().pipe(createDecimalStringValidator({
        allowZero: false,
...
export { BalanceError, Blockchain, InputError, KitError, NetworkError, OnchainError, RpcError, SUPPORTED_TOKENS, UnifiedBalanceChain, UnifiedBalanceKit, addDelegate, createUnifiedBalanceKitContext, deposit, depositFor, estimateSpend, getBalances, getDelegateStatus, getErrorCode, getErrorMessage, getSupportedChains, initiateRemoveFund, isBalanceError, isFatalError, isInputError, isKitError, isNetworkError, isOnchainError, isRetryableError$1 as isRetryableError, isRpcError, removeDelegate, removeFund, resolveChainIdentifier, setExternalPrefix, spend };
