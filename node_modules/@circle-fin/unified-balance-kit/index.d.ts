...
/**
 * Internal context object that holds the resolved state for a
 * {@link UnifiedBalanceKit} instance.
 *
 * In the class-based API this context is held privately inside the class.
 * In the functional API consumers create one via
 * {@link createUnifiedBalanceKitContext} and pass it explicitly to every
 * operation function.
 *
 * The generic type parameter preserves the exact types of all providers
 * (default + custom) for type safety.
 *
 * @typeParam TProviders - Array type of all providers in the context
 *
 * @example
 * ```typescript
 * import {
 ...
 * ```
 */
interface UnifiedBalanceKitContext<TProviders extends readonly FlexibleGatewayProvider[] = readonly FlexibleGatewayProvider[]> {
    /** Gateway providers available for routing operations. */
    providers: TProviders;
    /** Optional custom fee policy for spend operations. */
 ...
 * ```
 */
declare function deposit<TAdapterCapabilities extends AdapterCapabilities = AdapterCapabilities>(context: UnifiedBalanceKitContext, params: DepositParams<TAdapterCapabilities>): Promise<DepositResult>;
/**
 ...
 * ```
 */
declare function depositFor<TAdapterCapabilities extends AdapterCapabilities = AdapterCapabilities>(context: UnifiedBalanceKitContext, params: DepositForParams<TAdapterCapabilities>): Promise<DepositResult>;
 ...
 * ```
 */
declare function spend<TFromAdapterCapabilities extends AdapterCapabilities = AdapterCapabilities, TToAdapterCapabilities extends AdapterCapabilities = AdapterCapabilities>(context: UnifiedBalanceKitContext, params: SpendParams<TFromAdapterCapabilities, TToAdapterCapabilities>): Promise<SpendResult>;
/**
 * Estimate the fees for a spend operation without executing it.
 *
 * @param context - The kit context containing providers.
 ...
 * ```
 */
declare function estimateSpend<TFromAdapterCapabilities extends AdapterCapabilities = AdapterCapabilities, TToAdapterCapabilities extends AdapterCapabilities = AdapterCapabilities>(context: UnifiedBalanceKitContext, params: SpendParams<TFromAdapterCapabilities, TToAdapterCapabilities>): Promise<EstimateSpendResult>;
 ...
 * @example
 * ```typescript
 * import type { ConfirmedBalanceEntry } from '@circle-fin/unified-balance-kit'
 *
 ...
 * ```
 */
declare function getBalances<TAdapterCapabilities extends AdapterCapabilities = AdapterCapabilities>(context: UnifiedBalanceKitContext, params: GetBalancesParams<TAdapterCapabilities>): Promise<GetBalancesResult>;
 ...
 * ```
 */
declare function addDelegate<TAdapterCapabilities extends AdapterCapabilities = AdapterCapabilities>(context: UnifiedBalanceKitContext, params: UpdateDelegateParams<TAdapterCapabilities>): Promise<UpdateDelegateResult>;
/**
 ...
 * ```
 */
declare function removeDelegate<TAdapterCapabilities extends AdapterCapabilities = AdapterCapabilities>(context: UnifiedBalanceKitContext, params: UpdateDelegateParams<TAdapterCapabilities>): Promise<UpdateDelegateResult>;
/**
 ...
 * ```
 */
declare function getDelegateStatus<TAdapterCapabilities extends AdapterCapabilities = AdapterCapabilities>(context: UnifiedBalanceKitContext, params: GetDelegateStatusParams<TAdapterCapabilities>): Promise<DelegateStatus>;
 ...
 * ```
 */
declare function initiateRemoveFund<TAdapterCapabilities extends AdapterCapabilities = AdapterCapabilities>(context: UnifiedBalanceKitContext, params: InitiateRemoveFundParams<TAdapterCapabilities>): Promise<InitiateRemoveFundResult>;
/**
 ...
 * ```
 */
declare function removeFund<TAdapterCapabilities extends AdapterCapabilities = AdapterCapabilities>(context: UnifiedBalanceKitContext, params: RemoveFundParams<TAdapterCapabilities>): Promise<RemoveFundResult>;
 ...
 * ```
 */
declare function getSupportedChains(context: UnifiedBalanceKitContext, token?: SupportedTokenInput, options?: GetSupportedChainsOptions): ChainDefinition[];
 ...
 * ```
 */
declare const getDefaultProviders: () => readonly [IGatewayProvider<GatewayV1Actions>];
/**
 * Type alias for the default providers returned by getDefaultProviders.
 *
 * Exported to allow proper type extraction from UnifiedBalanceKitContext.
 */
 ...
 * ```
 */
declare function createUnifiedBalanceKitContext<TExtraProviders extends FlexibleGatewayProvider[] = []>(config?: UnifiedBalanceKitConfig<TExtraProviders>): UnifiedBalanceKitContext<[...DefaultProviders, ...TExtraProviders]>;

/**
 * Merge action maps from a readonly tuple of providers into a single map.
 *
 * For each action name that appears in *any* provider, the resulting type
 * is a union of the corresponding payload types across all providers.
 *
 * @typeParam Ps - Readonly array of gateway providers.
 */
type MergeActions<Ps extends readonly FlexibleGatewayProvider[]> = {
    [K in keyof Ps[number]['actions']]: Ps[number]['actions'][K];
};
/**
 * Complete action map combining default providers with extra providers.
 *
 * @typeParam EP - Array of extra gateway providers beyond the defaults.
 */
type AllActions<EP extends FlexibleGatewayProvider[]> = MergeActions<[
    ...DefaultProviders,
    ...EP
]>;
/**
 ...
 * @typeParam EP - Array of extra gateway providers beyond the defaults.
 */
type ActionName<EP extends FlexibleGatewayProvider[]> = keyof AllActions<EP>;
/**
 * Type for event handler functions that can be registered with the kit.
 *
 * @typeParam T - The specific UnifiedBalanceKit instance type.
 *
 * @example
 * ```typescript
 * const kit = new UnifiedBalanceKit()
 ...
 * ```
 */
type ActionHandler<T extends UnifiedBalanceKit> = Parameters<T['on']>[1];
/**
 * Loosely-typed event subscription interface for wrapping code.
 *
 * Strongly-typed consumers should call `kit.on(...)` / `kit.off(...)`
 * directly and benefit from the overloaded signatures. This interface
 * exists so that higher-level wrappers (e.g. AppKit) can forward
 * arbitrary action strings without casting through `unknown`.
 *
 * @example
 * ```typescript
 * import type { UntypedEventEmitter } from '@circle-fin/unified-balance-kit'
 *
 * class Wrapper {
 *   private readonly emitter: UntypedEventEmitter
 *
 *   constructor(kit: UnifiedBalanceKit) {
 *     this.emitter = kit
 *   }
 *
...
interface UntypedEventEmitter {
    /** Register an event handler for a named action or the `'*'` wildcard. */
    on(action: string, handler: (payload: unknown) => void): void;
 ...
 * ```
 */
declare class UnifiedBalanceKit<TExtraProviders extends FlexibleGatewayProvider[] = []> implements UntypedEventEmitter {
     ...
     */
    constructor(config?: UnifiedBalanceKitConfig<TExtraProviders>);
    /**
     * Extract comma-separated source chain names from spend params.
     *
     * @internal
     */
    private static extractSpendSourceChains;
    /**
     * Register an event handler for a specific gateway lifecycle action.
     *
     * Subscribe to events emitted during gateway operations such as deposit,
     * spend, getBalances, etc. Handlers receive strongly-typed payloads
     * based on the action name.
     *
     * Multiple handlers can be registered for the same action, and all will
     * be invoked when the action occurs. Use the wildcard `'*'` to listen
     * to all actions.
     *
     * **Note:** TypeScript autocomplete may only show `'*'` due to internal
     * type erasure. The following action names are available at runtime and
     * can be imported as `GatewayActionName` from
     ...
     * @example
     * ```typescript
     * import type { GatewayActionName } from '@circle-fin/provider-gateway-v1'
     *
     * const kit = new UnifiedBalanceKit()
     *
     * kit.on('gateway.deposit.started' as GatewayActionName, (payload) => {
     *   console.log('Deposit started:', payload.tags.opName)
     * })
     *
     * kit.on('gateway.spend.succeeded' as GatewayActionName, (payload) => {
     *   console.log('Spend succeeded:', payload.data.txHash)
     * })
     *
     * kit.on('*', (payload) => {
     *   console.log('Event:', payload.tags.opName)
     * })
     * ```
     */
    on<K extends ActionName<TExtraProviders>>(action: K, handler: (payload: AllActions<TExtraProviders>[K]) => void): void;
     ...
     * same handler function reference that was used during registration.
     *
     ...
     * kit.on('gateway.deposit.succeeded', handler)
     *
     * // Later, unregister
     * kit.off('gateway.deposit.succeeded', handler)
     * ```
     */
    off<K extends ActionName<TExtraProviders>>(action: K, handler: (payload: AllActions<TExtraProviders>[K]) => void): void;
     ...
     * @returns Promise resolving to the aggregated balance result.
     */
    getBalances<TAdapterCapabilities extends AdapterCapabilities = AdapterCapabilities>(params: GetBalancesParams<TAdapterCapabilities>): Promise<GetBalancesResult>;
    /**
     * Grant spending rights to another address on the owner's account.
     *
     * @param params - The owner's adapter context and the delegate address
     ...
     * @returns Array of supported chain definitions.
     */
    getSupportedChains(token?: SupportedTokenInput, options?: {
        forwarderSupported?: 'source' | 'destination';
    }): ChainDefinition[];
    /**
...
export { BalanceError, Blockchain, InputError, KitError, NetworkError, OnchainError, RpcError, SUPPORTED_TOKENS, UnifiedBalanceChain, UnifiedBalanceKit, addDelegate, createUnifiedBalanceKitContext, deposit, depositFor, estimateSpend, getBalances, getDelegateStatus, getErrorCode, getErrorMessage, getSupportedChains, initiateRemoveFund, isBalanceError, isFatalError, isInputError, isKitError, isNetworkError, isOnchainError, isRetryableError, isRpcError, removeDelegate, removeFund, resolveChainIdentifier, setExternalPrefix, spend };
export type { ActionHandler, AdapterContext, Allocation, AllocationResult, AllowanceStrategy, BalanceSource, BalanceWithPendingBreakdown, ChainBalanceBreakdown, ChainDefinition, ChainIdentifier, ConfirmedBalanceEntry, CustomFeeConfig, CustomFeePolicy, DelegateStatus, DepositForParams, DepositParams, DepositResult, ErrorDetails, EstimateSpendResult, FeeAllocation, FeeEntry, FeeType, ForwarderSpendDestination, GetBalancesParams, GetBalancesResult, GetDelegateStatusParams, GetSupportedChainsOptions, InitiateRemoveFundParams, InitiateRemoveFundResult, NetworkType, PendingBalanceTransaction, Recoverability, RemoveFundParams, RemoveFundResult, ResolvedSpendParams, RetryMintConfig, SpendConfig, SpendDestination, SpendDestinationUnion, SpendFeeFunction, SpendFeeRecipientFunction, SpendParams, SpendResult, SpendSource, SpendStep, SupportedToken, SupportedTokenInput, UnifiedBalanceChainIdentifier, UnifiedBalanceKitConfig, UnifiedBalanceKitContext, UntypedEventEmitter, UpdateDelegateParams, UpdateDelegateResult };
