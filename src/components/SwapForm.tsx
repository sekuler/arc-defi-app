...
export default function SwapForm({ provider, address, balances, onRefresh }: Props) {
  ...
  useEffect(() => {
    async function loadMarketInfo() {
      ...
      try {
        const res = await fetch(`https://testnet.arcscan.app/api?module=account&action=txlist&address=${SWAP_CONTRACT}&limit=6`);
        const data = await res.json();
        const items: ContractTx[] = (data.result ?? []).slice(0, 6).map((tx: any) => ({
          hash: tx.hash,
